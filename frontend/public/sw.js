...
function isNavigationRequest(request, url) {
  return request.mode === 'navigate' && url.origin === self.location.origin && !/\.[a-zA-Z0-9]+$/.test(url.pathname);
}

function isStaticAsset(url) {
  return url.origin === self.location.origin && (url.pathname.startsWith('/assets/') || url.pathname.endsWith('.css') || url.pathname.endsWith('.js') || url.pathname.endsWith('.woff2'));
}
...
function shouldCacheGraphql(payload) {
  ...
  return operationName === 'GetFriends' || operationName === 'FriendLedger' || operationName === 'DashboardSummary' || query.includes('friendLedger') || query.includes('dashboardSummary');
}

function graphqlCacheKey(url, payload) {
  return new Request(`${url.origin}/__graphql_cache__?op=${encodeURIComponent(String(payload?.operationName || 'anonymous'))}&vars=${encodeURIComponent(JSON.stringify(payload?.variables || {}))}`);
}
