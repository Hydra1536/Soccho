...
export function EqualPayDrawer({ isOpen, onClose }: EqualPayDrawerProps) {
	...
	const ensureCount = (nextCount: number) => {
		setPeople((prev) => {
			...
			const extra = Array.from({ length: normalized - prev.length }, () => ({
				name: "",
				amount: "",
			}));
			return [...prev, ...extra];
					...
					>
						...
						<div className="p-4 space-y-4">
							<div className="bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl p-3 text-sm text-[#374151]">
								<p>
									একসাথে ঘোরাঘুরি বা খাওয়া-দাওয়ার খরচ সমানভাবে ভাগ করা নিয়ে চিন্তিত? কে কত টাকা
									ফেরত পাবে বা কাকে কত দিতে হবে, তার একটি পরিষ্কার তালিকা পেতে সবার নাম
									এবং কে কত খরচ করেছে তা যোগ করুন।
								</p>
							...
							<div className="bg-white border border-[#E5E7EB] rounded-xl p-4">
								...
								<div className="flex items-center gap-2">
									<input
										...
										value={countInput}
										onChange={(event) =>
											setCountInput(event.target.value.replace(/\D/g, ""))
										}
								...
								{people.slice(0, peopleCount).map((person, idx) => (
									...
									>
										...
										<input
											...
											value={person.name}
											onChange={(event) =>
												handlePersonChange(idx, "name", event.target.value)
											}
										...
										/>
										<label className="block text-sm text-[#111827] mt-3 mb-2 font-medium">
											Paid Amount (Taka)
										</label>
										<input
											type="text"
											...
											value={person.amount}
											onChange={(event) =>
												handlePersonChange(idx, "amount", event.target.value)
											}
											className="w-full h-10 px-3 bg-[#F3F4F6] border border-[#E5E7EB] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4F46E5] focus:border-[#4F46E5]"
										/>
									</div>
							...
							{result && (
								...
								>
									<h3 className="font-bold text-[#111827]">
										Average Payment: {result.avg} taka
									</h3>
									{result.settlements.length === 0 && (
										<p className="text-sm text-[#6B7280] mt-2">
											Everyone already paid equally.
										</p>
									)}
									{result.settlements.map((item, index) => (
										<p key={index} className="text-sm text-[#374151] mt-3">
											<strong>{item.from}</strong> will pay{" "}
											<strong>{item.amount}</strong> taka to{" "}
