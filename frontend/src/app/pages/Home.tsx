...
export default function Home() {
	...
	const [nextFriendsCursor, setNextFriendsCursor] = useState<string | null>(
		null,
	);
	...
	const {
		...
		error: summaryError,
	} = useQuery<{ dashboardSummary: DashboardSummaryNode }>(
		GET_DASHBOARD_SUMMARY,
		{
	...
	const summary =
		...
		previousSummaryData?.dashboardSummary ||
		null;
	const isOffline =
		...
		const buildCard = async (
			...
			try {
				const { data } = await apolloClient.query({
					...
					variables: {
						friendshipId: toDeterministicFriendshipUuid(
							String(friend.id || ""),
						),
					},
					context: { service: "transaction" },
				...
				pendingReceivable = Number(
					cached?.friendLedger?.pendingReceivable || 0,
				);
				pendingPayable = Number(cached?.friendLedger?.pendingPayable || 0);
			}

			const pendingTotal =
				Math.abs(pendingReceivable) + Math.abs(pendingPayable);
		...
		const fetchFriendPage = async (cursor: string | null, append: boolean) => {
			...
			try {
				...
				const rows = Array.isArray(data?.results)
					? (data.results as FriendApiRow[])
					: [];
				const built = await Promise.all(
					rows.map((row, idx) => buildCard(row, idx)),
				);
				if (cancelled) {
					return;
				}
	...
	const handleLoadMoreFriends = async () => {
		...
		setFriendsLoadingMore(true);
		try {
			const { data } = await api.get("/api/social/list/", {
				params: { cursor: nextFriendsCursor },
			});
			const rows = Array.isArray(data?.results)
				? (data.results as FriendApiRow[])
				: [];
			const built = await Promise.all(
				rows.map(async (friend, idx) => {
					...
					try {
						...
						pendingReceivable = Number(
							ledgerData?.friendLedger?.pendingReceivable || 0,
						);
						pendingPayable = Number(
							ledgerData?.friendLedger?.pendingPayable || 0,
						);
					} catch {
						const cached = apolloClient.readQuery({
							query: GET_FRIEND_LEDGER,
	...
	const pieData = useMemo(
		() => [
			{
				name: "Given",
				value: Number(summary?.totalLent || 0),
				color: "#4F46E5",
			},
			{
				name: "Received",
				value: Number(summary?.totalBorrowed || 0),
				color: "#818CF8",
			},
		],
		[summary],
		...
		<div className="min-h-screen bg-[#F3F4F6] pb-20">
			<div className="bg-white border-b border-[#E5E7EB] sticky top-0 z-10">
				<div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
					<button
						onClick={() => setEqualPayOpen(true)}
						className="p-2 hover:bg-[#F3F4F6] rounded-lg transition-colors"
						aria-label="Open EqualPay Calculator"
					>
						<Calculator size={24} />
					</button>
			...
			<div className="max-w-md mx-auto px-4 py-6 space-y-6">
				<div className="bg-white rounded-2xl p-6 shadow-sm">
					...
					{summaryLoading && !summary && (
						<p className="text-sm text-[#6B7280] mb-4">Loading summary...</p>
					)}
					...
					<div className="mt-3 rounded-lg bg-[#EEF2FF] border border-[#C7D2FE] p-3">
						...
						<p className="text-lg font-semibold text-[#1E1B4B]">
							{Number(summary?.loyaltyScore || 0).toFixed(1)} / 100
						</p>
					</div>
					<div className="grid grid-cols-3 gap-2 mt-4">
						<div className="rounded-lg bg-[#F9FAFB] border border-[#E5E7EB] p-3 text-center">
							...
							<p className="text-sm font-semibold text-[#111827]">
								TK {Number(summary?.totalLent || 0).toLocaleString()}
							</p>
						</div>
						<div className="rounded-lg bg-[#F9FAFB] border border-[#E5E7EB] p-3 text-center">
							<p className="text-xs text-[#6B7280]">Received</p>
							<p className="text-sm font-semibold text-[#111827]">
								TK {Number(summary?.totalBorrowed || 0).toLocaleString()}
							</p>
						</div>
						<div className="rounded-lg bg-[#F9FAFB] border border-[#E5E7EB] p-3 text-center">
							<p className="text-xs text-[#6B7280]">Confirmed</p>
							<p className="text-sm font-semibold text-[#111827]">
								{Number(summary?.totalConfirmed || 0).toLocaleString()}
							</p>
						</div>
					</div>
				</div>
				...
				<div>
					...
					{friendsLoading && friendCards.length === 0 && (
						<p className="text-sm text-[#6B7280] mb-3">
							বন্ধুদের তথ্য লোড হচ্ছে...
						</p>
					)}
					{friendsErrorMessage && (
						<p className="text-sm text-[#B45309] mb-3">{friendsErrorMessage}</p>
					)}
					{isOffline && friendCards.length > 0 && (
						<p className="text-xs text-[#6B7280] mb-3">
							অফলাইন মোড: সেভ করা ডেটা দেখানো হচ্ছে।
						</p>
					)}
