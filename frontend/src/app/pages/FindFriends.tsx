...
export default function FindFriends() {
	...
	const [pendingActionInFlight, setPendingActionInFlight] = useState<
		Record<string, boolean>
	>({});
	...
	const fetchSearchResults = async (query: string) => {
		...
		try {
			const { data } = await api.get("/api/social/search/", {
				params: { q: query },
			});
	...
	const fetchPendingRequests = async () => {
		...
		try {
			...
			const outgoing = Array.isArray(data?.outgoing)
				? (data.outgoing as PendingRequestRow[])
				: [];
			const incoming = Array.isArray(data?.incoming)
				? (data.incoming as PendingRequestRow[])
				: [];
			const outgoingMap = outgoing.reduce<Record<string, boolean>>(
				(acc, row) => {
	...
	const fetchAcceptedFriends = async () => {
		...
		try {
			while (safety < 50) {
				...
				const { data } = await api.get<FriendListResponse>(
					"/api/social/list/",
					{ params },
				);
				...
				rows.forEach((row) => {
					...
					const counterpartId = String(
						row.counterpart_id || fallbackCounterpartId || "",
					).trim();
	...
	const handleIncomingAction = async (
		requestRow: PendingRequestRow,
		action: "accept" | "reject",
	) => {
		const requesterId = String(
			requestRow.counterpart_id || requestRow.requester_id || "",
		).trim();
		if (!requesterId) {
			return;
		}
		...
		try {
			const endpoint =
				action === "accept" ? "/api/social/accept/" : "/api/social/reject/";
			await api.post(endpoint, { user_id: requesterId });
			setIncomingRequests((prev) =>
				prev.filter((row) => String(row.id) !== requestId),
			);
			void fetchPendingRequests();
			void fetchAcceptedFriends();
		} catch (error) {
		...
		<div className="min-h-screen bg-[#F3F4F6] pb-20">
			...
			<div className="max-w-md mx-auto px-4 py-6 space-y-6">
				<div>
					...
					{pendingLoading && (
						<p className="text-sm text-[#6B7280] mb-3">Loading requests...</p>
					)}
					{!pendingLoading && incomingRequests.length === 0 && (
						<p className="text-sm text-[#6B7280] mb-3">No incoming requests.</p>
					)}
							...
							return (
								...
								>
									<div className="flex items-center gap-3">
										<Avatar
											name={row.counterpart_username || "User"}
											size="medium"
										/>
										<div className="flex-1">
											<h3 className="font-medium text-[#111827]">
												{row.counterpart_username || "Unknown user"}
											</h3>
				...
				{showResults && (
					<div>
						...
						{isSearching && (
							<p className="text-sm text-[#6B7280] mb-3">Searching...</p>
						)}
						{searchErrorMessage && (
						...
						<div className="space-y-3">
							{searchResults.map((person, idx) => {
								...
								const friendshipId = targetId
									? acceptedFriendships[targetId]
									: "";
								const isAlreadyFriend = !!friendshipId;
								const isRequested = targetId
									? !!requestedUserIds[targetId]
									: false;
								const isLoading = targetId
									? !!requestInFlight[targetId]
									: false;
								...
								return (
									...
									>
										<div className="flex items-center gap-3">
											<Avatar
												name={
													person.username || person.name || `Friend ${idx + 1}`
												}
												size="medium"
											/>
											<div className="flex-1">
