...
def mark_agreed(transaction_obj: Transaction) -> tuple[Transaction, DueRecord]:
    ...
    transaction_obj.save(update_fields=['status', 'verified_at', 'rejected_at', 'updated_at'])
...
def mark_rejected(transaction_obj: Transaction) -> Transaction:
    ...
    transaction_obj.save(update_fields=['status', 'rejected_at', 'verified_at', 'updated_at'])
...
def apply_repayment_fifo(friendship_id, payer_id, payee_id, amount: Decimal):
    ...
    rows = list(
        DueRecord.objects.select_for_update()
        .filter(
    ...
    outstanding = sum((Decimal(row.remaining_amount) for row in rows), start=Decimal('0'))
    ...
    for row in rows:
        ...
        row.save(update_fields=['remaining_amount', 'status', 'settled_at', 'updated_at'])
        allocations.append(
            {
