...
@router.post(
    "/", response={201: TransactionOut, 200: TransactionOut, 403: dict, 409: dict}
)
def create_transaction(request: HttpRequest, payload: TransactionCreateIn):
    ...
    acquired = client.set(
        lock_key, "1", nx=True, ex=int(timedelta(hours=24).total_seconds())
    )
    if not acquired:
...
@router.post(
    ...
    response={200: TransactionOut, 403: dict, 404: dict, 409: dict},
)
def resolve_transaction(
    request: HttpRequest, transaction_id: str, payload: ResolveTransactionIn
):
    ...
    with db_transaction.atomic():
        ...
        if action == "agree":
            ...
            borrower_name = _requester_username(request) or _user_name(
                str(tx.borrower_id)
            )
            _publish(
            ...
            _hard_evict_related_caches(
                str(tx.friendship_id), [str(tx.borrower_id), str(tx.lender_id)]
            )
            return 200, tx
...
def create_repayment(request: HttpRequest, payload: RepaymentCreateIn):
    ...
    if not acquired:
        existing = Repayment.objects.filter(
            idempotency_key=payload.idempotency_key
        ).first()
        if existing is not None:
            return 200, {
                "repayment_id": str(existing.id),
                "detail": "Repayment already recorded",
            }
        return 409, {"detail": "Duplicate idempotency key"}
    ...
    with db_transaction.atomic():
        try:
            allocation = apply_repayment_fifo(
                payload.friendship_id, payload.payer_id, payload.payee_id, amount
            )
    ...
    _hard_evict_related_caches(
        str(payload.friendship_id), [str(payload.payer_id), str(payload.payee_id)]
    )
...
def loyalty_score(request: HttpRequest):
    ...
    try:
        ...
        computed = type(
            "ScoreFallback",
            (),
            {"loyalty_score": 0.0, "total_lent": 0.0, "total_borrowed": 0.0},
        )()
    return 200, {
        "user_id": user_id,
