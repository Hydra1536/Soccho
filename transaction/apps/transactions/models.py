...
class Transaction(models.Model):
    ...
    status = models.CharField(
        max_length=32, choices=STATUS_CHOICES, default=STATUS_PENDING_VERIFICATION
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["friendship_id", "status"], name="txn_friend_status_idx"
            ),
            models.Index(
                fields=["borrower_id", "status"], name="txn_borrower_status_idx"
            ),
            models.Index(
                fields=["lender_id", "status", "created_at"],
                name="txn_lender_status_created_idx",
            ),
            models.Index(
                fields=["borrower_id", "status", "created_at"],
                name="txn_borr_sts_crtd_idx",
            ),
...
class DueRecord(models.Model):
    ...
    transaction = models.OneToOneField(
        Transaction, on_delete=models.CASCADE, related_name="due_record"
    )
    lender_id = models.UUIDField()
    borrower_id = models.UUIDField()
    friendship_id = models.UUIDField(db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["borrower_id", "status", "due_date"],
                name="due_borrower_status_due_idx",
            ),
            models.Index(
                fields=["lender_id", "status", "due_date"],
                name="due_lender_status_due_idx",
            ),
            models.Index(
                fields=["friendship_id", "status"], name="due_friend_status_idx"
            ),
        ]
...
class Repayment(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["friendship_id", "created_at"], name="repay_friend_created_idx"
            ),
            models.Index(
                fields=["payer_id", "created_at"], name="repay_payer_created_idx"
            ),
        ]
